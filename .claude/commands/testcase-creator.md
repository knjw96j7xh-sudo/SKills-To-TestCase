...
2. 收到选择后，生成结构化摘要并写入 `.testcase-assets/history/0-用例准备_<timestamp>.md`：
...
将用例表写入 `.testcase-assets/history/1-评审记要_<timestamp>.md`。
...
2. 针对每个选中评审点，逐条判断覆盖情况，输出评审报告：

```markdown
##  独立视角评审报告

| 评审点 | 是否已覆盖 | 分析说明 | 建议补充用例 |
|--------|------------|----------|--------------|
| UX-01 | [OK] 已覆盖 | TC-002 已验证错误提示 | — |
| BUG-01 | [FAIL] 未覆盖 | 无切换账号场景 | 补充 TC-005 |
| COMP-02 | [WARN] 部分覆盖 | 缺少空字符串边界 | 补充 TC-006 |
...
4. 评审通过后将最终用例表追加记录到 `1-评审记要_<timestamp>.md`。
...
1. 输出完整最终用例表，写入 `.testcase-assets/history/2-用例定稿_<timestamp>.md`
...
是否需要生成 OTP 导入文件？
  Y. 生成 OTP 树形 JSON（参考 .testcase-assets/templates/otp-schema.json）
  N. 不需要，跳过 OTP
```
...
3. **若选 Y**：
   ...
   - 输出文件 `otp_export_<timestamp>.json` 并展示内容预览
...
**步骤 A — 序列化用例数据**：将最终用例以如下 JSON 格式写入中间文件 `.testcase-assets/history/export_data_<timestamp>.json`：
