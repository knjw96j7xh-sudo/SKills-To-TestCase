...
2. **独立视角评审（subagent 模式）**：

   用户选择评审维度后，**启动隔离上下文的 subagent** 执行评审，模拟「换一个人审」的效果：
   - subagent 接收以下上下文：需求要素摘要 + 用例表 + 已选评审点列表
   - subagent 以独立评审人视角逐条判断覆盖情况，不受生成用例时的思维定势影响
   - subagent 将评审报告写入 `.testcase-assets/history/<运行目录>/1-评审记要.md`
   - 主 agent 读取评审报告后展示给用户

   > **为什么要用 subagent**：同一上下文生成的用例再自己审，容易有确认偏误。隔离上下文的 subagent 没有生成时的记忆，能以全新视角发现遗漏。
